...
import { fetchPaginationUpcoming } from "@/libs/fetch-api";
...
const UpcomingClient = () => {
  ...
  const fetchDataPopular = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await fetchPaginationUpcoming(page);
      setPopular(response);

      if (response?.data?.length) {
        const newRandomPage = getRandomIntInclusive(
          0,
  ...
  return (
    <section>
      {popular?.data?.[randomPage] && (
        <Banner
          title={popular?.data?.[randomPage]?.title}
          subtitle={getSynopsis}
          variant="image"
