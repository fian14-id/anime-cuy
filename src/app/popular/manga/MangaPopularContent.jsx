...
import { fetchPaginationMangaPopular } from "@/libs/fetch-api";
import { useState, useEffect, useCallback } from "react";

const MangaPopularContent = () => {
    ...
    const fetchDataPopular = useCallback(async () => {
        try {
            setIsLoading(true);
            const response = await fetchPaginationMangaPopular(page);
            setPopular(response);
    ...
    return (
        <section>
            {popular?.data?.[randomPage] && <Banner title={popular?.data?.[randomPage]?.title} subtitle={getSynopsis} variant="image" image={getImageRandom} className="text-palette-primary" />}
            {isLoading ? (
                <SkeletonLoading />
            ) : (
                <AnimeList 
...
export default MangaPopularContent;